...
async def cmd_start(message: types.Message):
    await message.answer(
        "👋 Привет! Я твой VPS-ассистент.\nИспользуй кнопки внизу для управления серверами.",
        reply_markup=get_main_keyboard()
    )

# Добавляем обработку как команды, так и текста на кнопке
...
async def start_add_server(message: types.Message, state: FSMContext):
    await message.answer("📝 Введите название сервера (напр. My-VPS):")
...
async def list_servers(message: types.Message):
    ...
    text += "\nЧтобы удалить, нажми кнопку '❌ Удалить сервер' или введи <code>/remove_server ID</code>"
    await message.answer(text)

@dp.message(F.text == "❌ Удалить сервер")
async def ask_remove_id(message: types.Message):
    await message.answer("Чтобы удалить сервер, введите команду с его ID.\nПример: <code>/remove_server 1</code>\n\n(ID можно узнать в списке серверов)")
...
async def remove_server(message: types.Message):
    try:
        ...
        await message.answer("Ошибка! Введите: <code>/remove_server ID</code>")

# --- SSH Логика (Безагентная) ---
...
def get_detailed_status(ip, user, password):
    try:
        ...
        # Используем ; вместо && и добавляем echo 'N/A', если команда упадет
        commands = (
            "uptime -p || echo 'N/A'; "
        ...
        # Логируем для отладки (увидите в docker logs)
        print(f"DEBUG SSH output from {ip}: {res}")

        # Заполняем данные с проверкой на наличие строк
        return {
            "online": True,
...
async def server_status(message: types.Message):
    ...
    status_msg = await message.answer("⌛ Подключаюсь к серверам...")
    
    final_report = []
    for s in servers:
        # s[0]=id, s[1]=name, s[2]=ip, s[3]=user, s[4]=password
        loop = asyncio.get_event_loop()
        data = await loop.run_in_executor(None, get_detailed_status, s[2], s[3], s[4])
        
        if data["online"]:
            report = (
                f"🖥 <b>{s[1]}</b> (<code>{s[2]}</code>)\n"
                f"✅ <b>Статус:</b> Online\n"
                f"⏱ <b>Uptime:</b> {data['uptime']}\n"
                f"📊 <b>Load:</b> {data['load']} | 📟 <b>{data['ram']}</b>\n"
                f"💾 <b>{data['disk']}</b>"
            )
        else:
            report = f"🖥 <b>{s[1]}</b> (<code>{s[2]}</code>)\n❌ <b>Ошибка:</b> <code>{data['error']}</code>"
        
        final_report.append(report)

    await status_msg.edit_text("\n\n---\n\n".join(final_report))

# --- Остальные шаги FSM (добавление сервера) ---
...
async def process_ip(message: types.Message, state: FSMContext):
    ...
    await message.answer("👤 Введите SSH имя пользователя (напр. root):")
